...
const sections = [
  { href: "/student/route", label: "Маршрут", title: "До экзамена", description: "План по неделям от точки старта до финальных вариантов.", accent: "bg-cyan-50 text-cyan-800" },
...
const toneByKind: Record<TodayItem["kind"], { label: string; badge: string; border: string }> = {
  OVERDUE: { label: "Просрочено", badge: "bg-rose-100 text-rose-800", border: "border-rose-200" },
  DUE_SOON: { label: "Скоро дедлайн", badge: "bg-amber-100 text-amber-800", border: "border-amber-200" },
  ASSIGNED: { label: "Назначено", badge: "bg-slate-100 text-slate-700", border: "border-slate-200" },
  PRACTICE: { label: "Слабый навык", badge: "bg-violet-100 text-violet-800", border: "border-violet-200" },
  WEBINAR: { label: "Вебинар", badge: "bg-cyan-100 text-cyan-800", border: "border-cyan-200" },
  ONBOARDING: { label: "Начните здесь", badge: "bg-cyan-100 text-cyan-800", border: "border-cyan-200" },
  DIAGNOSTIC: { label: "Точка старта", badge: "bg-indigo-100 text-indigo-800", border: "border-indigo-200" },
  ROADMAP: { label: "Маршрут", badge: "bg-emerald-100 text-emerald-800", border: "border-emerald-200" },
  ERROR_REVIEW: { label: "Исправление", badge: "bg-rose-100 text-rose-800", border: "border-rose-200" },
  RECOVERY: { label: "Восстановление", badge: "bg-fuchsia-100 text-fuchsia-800", border: "border-fuchsia-200" },
};
...
export default async function StudentPage() {
  ...
  const [todayItems, upcomingWebinars, studyPlanRecord, journey] = await Promise.all([
  ...
  const startSteps = [Boolean(journey.profile), journey.diagnostic?.status === "COMPLETED", Boolean(journey.roadmap)];
      ...
      <div className="mx-auto max-w-[1240px]">
        ...
        <section className="mt-5 overflow-hidden rounded-[32px] bg-[#092535] px-6 py-7 text-white shadow-xl sm:px-8 lg:px-10 lg:py-9">
          <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
            <div><div className="font-mono text-[11px] uppercase tracking-[0.18em] text-cyan-300">dashboard.today</div><h1 className="mt-3 text-3xl font-black tracking-[-0.04em] sm:text-5xl">{firstName}, вот ваш следующий понятный шаг.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">Очередь учитывает старт, дедлайны, маршрут, ошибки и слабые навыки. У каждого пункта есть причина и действие — выбирать наугад не нужно.</p></div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <ol className="mt-4 space-y-2 text-xs">{["Цель и ресурс", "Входная диагностика", "Маршрут до экзамена"].map((label, index) => <li key={label} className={startSteps[index] ? "text-emerald-300" : "text-slate-300"}>{startSteps[index] ? "✓" : `${index + 1}.`} {label}</li>)}</ol>
              {completedStartSteps < 3 ? <Link href={!journey.profile ? "/student/start" : "/student/diagnostic"} className="mt-4 inline-flex rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-black text-slate-950">Продолжить настройку →</Link> : <p className="mt-4 text-xs leading-5 text-emerald-200">Маршрут готов и будет обновляться по новым независимым результатам.</p>}
            </div>
          </div>
        </section>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(330px,0.65fr)]">
          <article className="rounded-[30px] border border-white bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-cyan-700">one.queue</div><h2 className="mt-2 text-2xl font-black">Сегодня</h2><p className="mt-1 text-sm text-slate-500">Сверху — самое важное. Дедлайн служит сигналом для действия или перепланирования.</p></div>
              <div className="flex gap-2">{overdueCount >= 2 || journey.recovery ? <Link href="/student/recovery" className="rounded-xl bg-fuchsia-50 px-3 py-2 text-xs font-black text-fuchsia-800">Режим восстановления</Link> : null}<Link href="/student/calendar" className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-black text-slate-700">Календарь</Link></div>
            </div>
            {todayItems.length === 0 ? (
              <div className="mt-5 rounded-2xl border border-dashed border-emerald-200 bg-emerald-50 p-6"><h3 className="text-lg font-black text-emerald-900">Очередь закрыта ✓</h3><p className="mt-2 text-sm leading-6 text-emerald-800">Можно закрепить навык из текущей недели маршрута или решить полный вариант.</p><Link href="/student/route" className="mt-4 inline-flex rounded-xl bg-emerald-900 px-4 py-2.5 text-sm font-bold text-white">Открыть маршрут</Link></div>
            ) : (
              <div className="mt-5 space-y-3">
                {todayItems.map((item, index) => {
                  const tone = toneByKind[item.kind];
                  return <article key={item.key} className={`rounded-2xl border ${tone.border} bg-slate-50/70 p-4 sm:p-5`}><div className="flex gap-4"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-sm font-black shadow-sm">{index + 1}</div><div className="min-w-0 flex-1"><span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold ${tone.badge}`}>{tone.label} · ~{item.estimatedMinutes} мин</span><h3 className="mt-2 text-base font-black sm:text-lg">{item.title}</h3><p className="mt-1 text-sm leading-6 text-slate-500">{item.description}</p><details className="mt-2 text-xs leading-5 text-slate-500"><summary className="cursor-pointer font-bold text-cyan-800">Почему это сейчас?</summary><p className="mt-1">{item.why}</p></details><TodayItemActions itemKey={item.key} href={item.href} checkHref={item.checkHref} action={item.action} external={item.kind === "WEBINAR"} /></div></div></article>;
                })}
              </div>
            )}
          </article>
          ...
          <aside className="space-y-5">
            <article className="rounded-[30px] border border-white bg-white p-5 shadow-sm sm:p-6"><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-cyan-700">global.route</div><h2 className="mt-2 text-2xl font-black">До экзамена</h2>{journey.profile ? <><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-cyan-50 p-4"><div className="text-xs text-cyan-700">Цель</div><div className="mt-1 text-2xl font-black text-cyan-950">{journey.profile.targetScore}+</div></div><div className="rounded-2xl bg-violet-50 p-4"><div className="text-xs text-violet-700">Ресурс</div><div className="mt-1 text-2xl font-black text-violet-950">{Math.round(journey.profile.weeklyMinutes / 60)} ч/нед.</div></div></div><div className="mt-4 flex flex-wrap gap-2"><Link href="/student/route" className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white">Весь маршрут</Link><Link href="/student/skills" className="rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-black text-slate-700">Карта навыков</Link></div></> : <><p className="mt-3 text-sm leading-6 text-slate-500">Укажите цель, дату экзамена и реальный недельный ресурс — после диагностики появится маршрут.</p><Link href="/student/start" className="mt-4 inline-flex rounded-xl bg-cyan-700 px-4 py-2.5 text-xs font-black text-white">Настроить старт</Link></>}</article>
        ...
        {studyPlan ? <section className="mt-5"><div className="mb-3 px-1"><h2 className="text-xl font-black">Ближайший спринт от учителя</h2><p className="mt-1 text-sm text-slate-500">Это короткий подтверждённый план. Он дополняет, но не заменяет глобальный маршрут.</p></div><StudyPlanSummaryCard plan={studyPlan} /></section> : null}
