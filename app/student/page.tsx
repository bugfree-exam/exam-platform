      ...
      <div className="mx-auto max-w-[1240px]">
        ...
        <section className="mt-5 overflow-hidden rounded-[32px] bg-[#092535] px-6 py-7 text-white shadow-xl sm:px-8 lg:px-10 lg:py-9">
          <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
            <div><div className="font-mono text-[11px] uppercase tracking-[0.18em] text-cyan-300">dashboard.today</div><h1 className="mt-3 text-3xl font-black tracking-[-0.04em] sm:text-5xl">{firstName}, вот план курса на сегодня.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">Здесь собраны пункты годового графика, сегодняшние вебинары и все назначенные ДЗ, которые ещё нужно сдать.</p></div>
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(330px,0.65fr)]">
          <article className="rounded-[30px] border border-white bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-cyan-700">course.schedule.today</div><h2 className="mt-2 text-2xl font-black">Сегодня</h2><p className="mt-1 text-sm text-slate-500">План курса на текущую дату, назначенные и просроченные домашние работы.</p></div>
            ...
            {todayItems.length === 0 ? (
              <div className="mt-5 rounded-2xl border border-dashed border-emerald-200 bg-emerald-50 p-6"><h3 className="text-lg font-black text-emerald-900">На сегодня всё выполнено ✓</h3><p className="mt-2 text-sm leading-6 text-emerald-800">По графику нет обязательных действий, а все назначенные домашние работы уже сданы.</p><Link href="/student/route" className="mt-4 inline-flex rounded-xl bg-emerald-900 px-4 py-2.5 text-sm font-bold text-white">Посмотреть маршрут</Link></div>
            ) : (
        ...
        {backlogItems.length > 0 ? <section className="mt-5 rounded-[28px] border border-amber-200 bg-amber-50 p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-amber-700">separate.backlog</div><h2 className="mt-2 text-xl font-black text-amber-950">Просроченные варианты</h2><p className="mt-1 text-sm leading-6 text-amber-800">Эти варианты показаны отдельно от ежедневного плана. Их можно закрыть самостоятельно или перейти в режим восстановления.</p></div><Link href="/student/recovery" className="rounded-xl bg-white px-4 py-2.5 text-xs font-black text-amber-900">Режим восстановления</Link></div><div className="mt-4 grid gap-3 md:grid-cols-2">{backlogItems.map((item) => <Link key={item.key} href={item.href} className="rounded-2xl border border-amber-200 bg-white p-4 transition hover:border-amber-400"><div className="text-xs font-bold text-amber-700">{item.description}</div><div className="mt-1 font-black text-slate-950">{item.title}</div></Link>)}</div></section> : null}
