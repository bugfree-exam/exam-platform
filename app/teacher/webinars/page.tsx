...
export default async function TeacherWebinarsPage({
  searchParams,
}: TeacherWebinarsPageProps) {
  const params = await searchParams;

  const selectedQuery = params.q?.trim() ?? "";
  const selectedStatus = VALID_STATUSES.includes(params.status ?? "")
    ? params.status ?? "all"
    : "all";
  ...
  if (selectedStatus !== "all") {
    where.status = selectedStatus as WebinarStatus;
  }
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-8 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-slate-600">
              Видеоразборы, записи занятий, конспекты, шпаргалки и материалы.
            </p>
          </div>

          <Link
            href="/teacher/webinars/create"
        ...
        {webinars.length === 0 ? (
          <section className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center">
            ...
            <p className="mt-2 text-slate-600">
              Попробуй изменить поиск, номер ЕГЭ, статус или период.
            </p>
          </section>
        ) : (
