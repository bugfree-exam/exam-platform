...
export function WebinarForm({ mode = "create", initialData }: WebinarFormProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="mt-5 space-y-4">
          <div>
            ...
            <select
              value={videoProvider}
              onChange={(event) =>
                setVideoProvider(event.target.value as WebinarVideoProvider)
              }
              className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10"
            >
          ...
          <div>
            ...
            <p className="mt-2 text-xs leading-5 text-slate-500">
              Для RuTube можно оставить пустым. Для Яндекс.Диска лучше вставить
              именно embed-ссылку, если она доступна.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Можно добавлять заголовки, списки, таблицы, картинки, ссылки и
          фрагменты кода.
        </p>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Ссылки на шпаргалки, презентации, документы, код и дополнительные
              материалы.
            </p>
          </div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateMaterial(
                      index,
                      "type",
                      event.target.value as WebinarMaterialType
                    )
                  }
